Given /^an ICCCM compliant window is mapped$/ do
  icccm_window_start
  Baf::Testing.wait_until message: 'window not mapped after %d seconds' do
...
When /^the(?:\s(\w+))? window is unmapped$/ do |ident|
  ...
  Baf::Testing.wait_until message: 'window not unmapped after %d seconds' do
...
Then /^the window must be focused$/ do
  Baf::Testing.wait_until message: 'window not focused after %d seconds' do
